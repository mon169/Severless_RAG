...
데이터 계층은 **Amazon S3 + LanceDB**를 사용하여  
전처리된 코퍼스 및 벡터 인덱스를 관리하며,  
**Amazon Bedrock Titan Embed Text v2**로 임베딩을 생성하고,  
**Anthropic Claude 3.5 Sonnet**으로 생성 결과를 도출합니다.  

Lambda 함수는 검색 시간, 전체 지연 시간, 콜드 스타트 여부, 비용 등을  
**CloudWatch**와 **X-Ray** 메트릭으로 기록하여 실험 결과를 집계합니다.  
...
1️⃣ **질문셋과 코퍼스 데이터의 일관성**이 충분히 확보되지 않을 경우  
검색 정확도 및 응답 품질이 저하될 수 있으며,  
이는 실험 결과의 일반화 가능성을 제한할 수 있습니다.  

2️⃣ **임베딩 모델의 차이**는 결과에 직접적인 영향을 미칩니다.  
